...
export class RuleEvaluator implements Evaluator {
  ...
  async evaluate(input: EvaluationInput): Promise<EvaluationResult> {
    ...
    const configCompetitors = Array.isArray(run.environment.experimentConfig?.competitors)
      ...
      : [];
    const competitorHosts = configCompetitors.map(hostOf).filter((h): h is string => Boolean(h));
    ...
    const customerDiscovered = Boolean(
      targetHost && (searchResultHosts.includes(targetHost) || navigatedHosts.includes(targetHost)),
    );

    const discoveredCompetitors = competitorHosts.filter(
      (h) => searchResultHosts.includes(h) || navigatedHosts.includes(h),
    );

    // Recommendation: who is named in the final answer?
    const customerRecommended = mentionsHost(answer, targetHost);
    const recommendedCompetitors = competitorHosts.filter((h) => mentionsHost(answer, h));
    const competitorRecommended = recommendedCompetitors.length > 0;
    ...
    const requiredInformationFound = Boolean(resultOk && answer);
    ...
    return {
      ...
      metrics: {
        ...
        recommended_competitors: recommendedCompetitors,
      },
    };
  }
}
