...
describe("PlaywrightBrowserProvider", () => {
  ...
  it("observes real CDP network events (success, redirect, error)", async () => {
    ...
    try {
      ...
      const requestEvents = networkEvents.filter(e => e.type === "network.request") as Array<{ type: string, payload: { url: string, requestId: string } }>;
      ...
      const responseEvents = networkEvents.filter(e => e.type === "network.response") as Array<{ type: string, payload: { url: string, status: number, requestId: string } }>;
      ...
      expect(redirectResponse!.payload.requestId).toBe(requestEvents.find(r => r.payload.url.endsWith("/redirect"))?.payload.requestId);
      expect(failResponse!.payload.requestId).toBe(requestEvents.find(r => r.payload.url.endsWith("/fail"))?.payload.requestId);
